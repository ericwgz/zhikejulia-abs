...
报告按本期/上期数值、阈值余量、情景现金流和具体数据缺口展开分析；正文数字由程序绑定填入，
模型负责选择重点、解释可能机制和提出针对性行动。升级后重新运行可生成新分析，历史报告不会自动改写。
提供3只合成ABS的180天样本、合约PDF/DOCX/TXT阈值提取、报告与现金流导出。
PDF支持文字版及扫描件，显示AI识别片段与页码，核对原PDF后才能应用阈值。
